...
function postItem(req, reply) {
    ...
    if (req.payload == null || req.payload.text == null) {
...
function deleteItem(req, reply) {
    ...
    return client.delAsync(id)
        .then ((num) => {
            ...
            if (num == 0) {
                return reply(boom.notFound('item (id=' + id.replace(/^id_/, "") +') is not exists.'));
            } else {
                reply({statusCode:200, message: 'item (id='+ id.replace(/^id_/, "") +') has been deleted.'});
            }
        });
}
